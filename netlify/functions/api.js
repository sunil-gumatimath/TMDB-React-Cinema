const handler = async (event) => {
  ...
  try {
    ...
    // Extract the API path - remove the function path prefix
    const apiPath = path.replace(/^\/\.netlify\/functions\/api/, '') || '/discover/movie';
    
    // Build TMDB URL
    let tmdbUrl = `https://api.themoviedb.org/3${apiPath}`;
    
    // Add query parameters if present
    if (queryStringParameters) {
      const params = new URLSearchParams(queryStringParameters);
      tmdbUrl += (tmdbUrl.includes('?') ? '&' : '?') + params.toString();
    }
